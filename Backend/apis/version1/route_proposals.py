...
from db.repository.products import create_new_proposals
from db.repository.products import delete_proposals_by_id
from db.repository.products import retreive_proposal
...
def read_proposal(id: int, db: Session = Depends(get_db)):
    job = retreive_proposal(id=id, db=db)
    if not job:
        raise HTTPException(
